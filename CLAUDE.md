...
**KEYPAD and JOYSTIK differ in shape — read both scripts before generalising.** In KEYPAD the resident driver is stored *plain* in the main image and disassembled inline via `add_move`; only the BASIC is encoded, and the loader is a *tail* (exec `&3906`). In JOYSTIK the loader is a *head* (exec `&1909`), and the resident driver ships as **two 256-byte configurable templates** (`&1A00`/`&1B00`, both running at `&0A00`) that live *inside* the ROL-encoded region — stored with their `joystick_map` keys, thresholds and BYTEV chain slot blank for the BASIC to poke. The JOYSTIK script disassembles each variant as its own dasmos instance, renders it to both `.asm` **and** `.json` (`…-driver-a/-b.*`), assembles it back with beebasm, then concatenates `driver A + driver B + tokenised BASIC` and ROR-encodes the lot into the `incbin` `.dat`. So JOYSTIK's `output/` holds **three** disassembly JSONs (main plus two variants); KEYPAD's holds one.
...
- `disassemblies` — *additional* formatted disassembly listings (JOYSTIK's two resident-driver templates), each pointing at one of the extra `output/*.json` files.
