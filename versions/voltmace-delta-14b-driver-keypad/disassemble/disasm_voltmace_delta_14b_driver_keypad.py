...
def build_basic_dat(bas_filepath):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # 'greedy' crunch, not the ROM tokeniser: these files were crunched
        # (normally tokenised, then non-string spaces stripped), leaving keywords
        # butted together (e.g. ENDPROCELSEPROC) that the ROM will not
        # re-tokenise. --crunch greedy reproduces that crunched end-state.
        # See build-and-verification.md section 5.
        subprocess.run(
...
ct(0x3933, 'redundant CLC: the ASL below sets carry from bit 7 itself')
...
ct(0x3953, "...'O' (&4F) is the OS 0.1 signature (the OS the BASIC rejects)")
ct(0x3955, 'OS 0.1 -> poke the buffer directly')
ct(0x3957, 'other (supported) OS -> insert via OSBYTE &8A')
ct(0x395A, 'Done')
